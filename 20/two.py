...
# The 12 possible transformations for each tile:
for tile in tiles:
	# It seems tiles aren't flipped horizontally
...
dimX = range(dim) if right == "r" else range(dim)[::-1]
dimY = range(dim) if up == "d" else range(dim)[::-1]
...
monsters = []
# Same as tiles: no horizontal flippin'
